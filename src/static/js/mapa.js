...
window.TrailMind.mapa = (function () {
  ...
  function inicializar() {
    ...
    var overlays = {
      "Senderos (Waymarked)": overlayWaymarked,
    };
  ...
  function toggleGradiente() {
    ...
    if (_modoGradiente) {
      ...
      if (_segmentosGradiente.length === 0) {
        _segmentosGradiente = _calcularSegmentosGradiente(_muestrear(_puntosActuales));
      }
  ...
  function _calcularSegmentosGradiente(puntos) {
    var resultado = [];
    if (puntos.length < 2) return resultado;

    var segmentoActual = [puntos[0]];
      ...
      if (color !== colorActual) {
        if (segmentoActual.length >= 2) {
          resultado.push(L.polyline(
            segmentoActual.map(function (p) { return [p.lat, p.lon]; }),
            { color: colorActual, weight: 4, opacity: 0.95 }
          ));
        }
        segmentoActual = [p1, p2];
        colorActual = color;
      } else {
        segmentoActual.push(p2);
      }
    }
